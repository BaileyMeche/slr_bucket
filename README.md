...
**Expected runtime:** ~2–4 minutes on a modern laptop.

**Key numbers to verify** (exit event, W=60, TOTAL spec):
- `coef_post_x_treas` = +3.84 bps
- `t_post_x_treas` = 4.32
- `n` = 2,057 observations
...
- Exit W=60 TOTAL: `t_DK` = 5.618, `se_DK` = 0.683
- Entry W=60 TOTAL: `t_DK` = -0.718
...
| Script | Key result |
|--------|-----------|
| task1 | CIP-AUD median = 12.55 bps (PASS); pooled without CIP-AUD = +3.81 bps |
| task2 | Pre-event F(6, .) = 9.92, p < 0.001 (relief-period contamination, not violation) |
| task3 | F(1, 12301) = 2.56, p = 0.11; 95% CI for mu2-mu4: [-3.86, +0.39] bps |
| task4 | 15/17 series AR(1) > 0.93; confirms HAC lags = 5 is appropriate |
| task5 | SOFR coef on non-Treasury spreads = -0.82 (t = -3.56) |
...
The draft uses `\input{../figures_output/figures.tex}` (or inline `\includegraphics`
calls pointing to `../figures_output/fig*.pdf`). Run pdflatex from the
`presentations/` directory so relative paths resolve correctly.

**Bibliography:** `presentations/references.bib` contains all 10 entries.
If pdflatex is run from the worktree root, adjust the `\bibliography{}` path.
...
### Missing table — `tab:did_clean`

`draft.tex` line 442 contains `Table~\ref{tab:did_clean}` referencing the clean
DiD estimates ($\hat{\mu}_2 = -10.76$ bps, $\hat{\mu}_4 = -9.03$ bps), but
**no `\label{tab:did_clean}` or `\begin{table}` environment exists in the draft**.
This will produce an undefined-reference warning at compile time and a `??` in
the PDF. The data is fully computed:

| Item | Value |
|------|-------|
| Source CSV | `audit_repairs/outputs/did_clean_baseline.csv` |
| Generated by | `audit_repairs/repaired_pipeline.py` (Step 4.5, clean DiD block) |
| Key columns | `spec`, `coef_relief`, `coef_relief_x_treas`, `se_relief_x_treas`, `t_relief_x_treas`, `coef_post_relief_x_treas`, `t_post_relief_x_treas`, `n`, `n_pre`, `n_relief`, `n_post`, `r2` |
| Rows | 2 rows: `spec=TOTAL` and `spec=DIRECT` |

To fix: add a `\begin{table}...\end{table}` environment with
`\label{tab:did_clean}` in Section IV (around line 490 of `draft.tex`),
populated from the two rows of `did_clean_baseline.csv`.
...
After running Steps 1-4, confirm:

```python
import pandas as pd
df = pd.read_csv("audit_repairs/outputs/jump_estimates_pooled.csv")
row = df[(df["event_date"]=="2021-03-31") & (df["window"]==60) & (df["spec"]=="TOTAL")]
print(row[["coef_post_x_treas","se_post_x_treas","t_post_x_treas","n"]].to_string())
# Expected: coef=3.84, se=0.89, t=4.32, n=2057
```

```python
df2 = pd.read_csv("audit_repairs/outputs/dk_robustness.csv")
row2 = df2[(df2["event_date"]=="2021-03-31") & (df2["window"]==60) & (df2["spec"]=="TOTAL")]
print(row2[["t_DK","se_DK"]].to_string())
# Expected: t_DK=5.618, se_DK=0.683
```

```python
df3 = pd.read_csv("audit_repairs/outputs/did_clean_baseline.csv")
row3 = df3[df3["spec"]=="TOTAL"]
print(row3[["coef_relief_x_treas","t_relief_x_treas","coef_post_relief_x_treas","t_post_relief_x_treas"]].to_string())
# Expected: relief=-10.76, t=-9.37; post_relief=-9.03, t=-7.79
```
