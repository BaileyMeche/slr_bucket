"""
make_figures.py — Publication figures for the SLR exclusion paper.
...
Outputs saved to _output/figures/ (PNG + PDF at 300 DPI).
Pipeline estimates read from _output/pipeline/.
"""
...
# src/slr_bucket/figures/make_figures.py → repo root is 3 levels up
REPO_ROOT  = Path(__file__).parents[3]
PIPELINE_OUT = REPO_ROOT / "_output" / "pipeline"
OUT_DIR    = REPO_ROOT / "_output" / "figures"
...
def load_panel():
    ...
    # Winsorize within-series at p1/p99
...
def label_slr_events(ax):
    ...
    ax.text(SLR_ENTRY + pd.Timedelta(days=5), 0.96, "Entry",
            transform=trans, color=COLORS["entry"], fontsize=9, fontweight="bold",
            va='top', ha='left', clip_on=True)
    ax.text(SLR_EXIT + pd.Timedelta(days=5), 0.96, "Exit",
            transform=trans, color=COLORS["exit"], fontsize=9, fontweight="bold",
            va='top', ha='left', clip_on=True)
...
# ── Figure 1 — Time-Series Overview (2x2) ────────────────────────────────────
...
def fig3_exit_event_paths(panel):
    ...
    for ax, pairs, panel_title in [
        ...
        ax.set_xlabel("Trading days relative to SLR exit", fontsize=12, fontweight="bold")
...
def fig4_coef_plot():
    ...
    pooled    = pd.read_csv(PIPELINE_OUT / "jump_estimates_pooled.csv")
    pooled["event"] = pd.to_datetime(pooled["event"])

    did_path  = PIPELINE_OUT / "did_clean_baseline.csv"
    ...
    for event_date, group_label, color in [
        ...
        for w in [60, 20]:
            for spec in ["TOTAL", "DIRECT"]:
                ...
                r = sub.iloc[0]
                rows.append({
                    "label": f"W={w}, {spec}", "coef": r["coef_post_x_treas"],
    ...
    for i, (r, y) in enumerate(zip(rows_h, yp)):
        ...
        if st:
            ax.text(ci_hi + 0.5, y, st, va='center', fontsize=8.5, color=r["color"])
...
def fig5_series_level_coefs():
    ...
    by_series = pd.read_csv(PIPELINE_OUT / "jump_estimates_by_series.csv")
    sub = by_series[
        (by_series["window"] == 60) &
    ...
    for _, row in ordered.iterrows():
        ...
        gc = next(c for s, g, c in GROUP_INFO if s == strat)
        rows_h.append({
            "sid": row["series_id"], "strat": strat,
            "coef": row["coef_post"], "se": row["se_post"], "t": row["t_post"],
...
def fig6_regime_boxplots(panel):
    ...
    for ax_idx, spec in enumerate(strategy_specs):
        ...
        for si, (sid, slabel) in enumerate(subseries):
            ...
            for ri, reg in enumerate(regimes):
                ...
                if isinstance(sid, list):
                    ...
                    vals = panel[(panel["series_id"] == sid) & (panel["regime"] == reg)]["y_abs_bps"]
        ...
        for patch, fc, ec in zip(bp['boxes'], face_colors, edge_colors):
            patch.set_facecolor(fc); patch.set_edgecolor(ec); patch.set_linewidth(0.9)
