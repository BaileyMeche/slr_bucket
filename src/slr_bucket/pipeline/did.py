"""
did.py — Panel difference-in-differences with pre-COVID 2019 baseline.

The event-window jump for entry (2020-04-01) uses Jan–Mar 2020 as its
pre-period, which spans the COVID crash peak. UST SF was dislocated by
100–300 bps during this window, so the pre-period baseline is artificially
elevated. This module implements a full-panel DiD using 2019 as a clean
pre-COVID reference.

Sample
------
  pre_period:  2019-01-01 to 2020-01-31  (pre-COVID, clean reference)
  relief:      2020-04-01 to 2021-03-31  (SLR exclusion active)
  post_relief: 2021-04-01 to 2021-12-31  (post-expiry)
  EXCLUDED:    2020-02-01 to 2020-03-31  (COVID crash — dropped)
...
Key coefficients
----------------
  relief_x_treas:       Treasury differential during SLR exclusion (DiD entry)
  post_relief_x_treas:  Treasury differential after expiry (DiD exit)
"""
...
def nw_fit(
    ...
    try:
        ...
        warnings.warn(f"nw_fit failed: {exc}")
        return None
...
def extract_coef(
    ...
    """Return (coef, SE, t-stat) for a named regressor; (NaN, NaN, NaN) if absent."""
    ...
    return coef, se, (coef / se if se > 0 else np.nan)


def build_did_sample(panel: pd.DataFrame) -> pd.DataFrame:
    """
    Construct the DiD sample with period indicators.
    Drops Feb–Mar 2020 (COVID crash). Adds: relief, post_relief, period.
    """
    df = panel.copy()
    df = df[df["date"].between("2019-01-01", "2021-12-31")].copy()
    df = df[~df["date"].between("2020-02-01", "2020-03-31")].copy()
    ...
    return df[df["period"] != "excluded"].copy()
...
def run_did_clean_baseline(
    ...
    spec_name : label for this specification ("TOTAL" or "DIRECT")
    ...
    if len(tb_vals) < 2:
        return {"spec": spec_name, "error": f"treasury_based is constant={tb_vals}"}
    ...
    keep = (
        ["y_abs_bps", "relief", "post_relief", "relief_x_treas",
    ...
    if n < 10 * k:
        warnings.warn(f"Low obs/param: {n}/{k} = {n/k:.1f}")
...
def run_did_both_specs(panel: pd.DataFrame) -> pd.DataFrame:
    ...
    for spec, ctrl in [("TOTAL", TOTAL_CONTROLS), ("DIRECT", DIRECT_CONTROLS)]:
        print(f"  Running DiD ({spec})...")
        result = run_did_clean_baseline(panel, ctrl, spec_name=spec)
        rows.append(result)
        if "error" not in result:
            print(
                f"    relief_x_treas:      {result['coef_relief_x_treas']:+.3f} "
                f"(SE={result['se_relief_x_treas']:.3f}, t={result['t_relief_x_treas']:.2f})"
            )
            print(
                f"    post_relief_x_treas: {result['coef_post_relief_x_treas']:+.3f} "
