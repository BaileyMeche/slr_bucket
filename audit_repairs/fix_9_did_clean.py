...
Interpretation:
  relief              = compression/expansion for non-Treasury series vs 2019
  relief:treasury_based  = DIFFERENTIAL compression for Treasury series (DiD estimate)
  post_relief         = mean-reversion for non-Treasury vs 2019
  post_relief:treasury_based = DIFFERENTIAL mean-reversion for Treasury (key exit DiD)
"""
...
def nw_fit(
    ...
    try:
        ...
        warnings.warn(f"[fix_9_did_clean] nw_fit failed: {exc}")
        return None
...
def run_did_clean_baseline(
    ...
    if n < 10 * k:
        warnings.warn(f"[fix_9_did_clean] Low obs/param: {n}/{k} = {n/k:.1f}")
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("FIX 9: Panel DiD with pre-COVID 2019 baseline")
    print("=" * 70)

    panel = load_full_panel()

    # Show sample composition
    did_panel = build_did_sample(panel)
    print(f"\nDiD sample: {len(did_panel):,} obs, {did_panel['series_id'].nunique()} series")
    for period, g in did_panel.groupby("period"):
        dates = g["date"].nunique()
        print(f"  {period:12s}: {len(g):6,} obs ({dates} dates, "
    ...
    print(f"\nSaved: {OUT_DIR}/did_clean_baseline.csv")
    ...
    for _, row in did_df.iterrows():
        if "error" in row:
            continue
        spec = row["spec"]
        rx = row["coef_relief_x_treas"]
        px = row["coef_post_relief_x_treas"]
        t_rx = row["t_relief_x_treas"]
        t_px = row["t_post_relief_x_treas"]
        print(f"\n  {spec} spec (N={row['n']:,}):")
        print(f"    Entry DiD (relief_x_treas): {rx:+.2f} bps (t={t_rx:.2f})")
        dir_entry = "compression (consistent with SLR relief)" if rx < 0 else "widening (unexpected)"
        print(f"    Direction: {dir_entry}")
        print(f"    Exit DiD (post_relief_x_treas): {px:+.2f} bps (t={t_px:.2f})")
        dir_exit = "widening (spread re-emerges after expiry)" if px > 0 else "further compression (unexpected)"
        print(f"    Direction: {dir_exit}")
