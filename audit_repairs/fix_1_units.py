...
multiply by 100. This incorrectly scales:
  - UST SF series whose stacked median is ~18 bps (< 20 threshold) → ×100 → ~1800 bps
  - CIP currencies with low spreads (AUD, CAD, GBP, NZD, ~10-13 bps) → ×100 → ~1200 bps
...
def load_cip_bps(path: Path) -> pd.DataFrame:
    ...
    print("[fix_1] CIP medians after fix (all should be 5-100 bps):")
...
def load_ust_sf_bps(path: Path, tenors: list[int] | None = None) -> pd.DataFrame:
    ...
    diag = long.groupby("series_id")["y_bps"].agg(["count", lambda x: x.abs().median()])
    diag.columns = ["n", "median_abs_bps"]
    print("[fix_1] UST SF medians after fix (should be 10-80 bps):")
    print(diag.to_string())
...
def load_tips_treas_bps(path: Path, tenors: list[int] | None = None) -> pd.DataFrame:
    ...
    diag = long.groupby("series_id")["y_bps"].agg(["count", lambda x: x.abs().median()])
    diag.columns = ["n", "median_abs_bps"]
    print("[fix_1] TIPS-Treasury medians after fix (should be 15-30 bps):")
    print(diag.to_string())
...
def load_equity_sf_bps(series_dir: Path, indices: list[str] | None = None) -> pd.DataFrame:
    ...
    for idx in indices:
        ...
        if not path.exists():
            print(f"[fix_1] WARNING: {path} not found, skipping")
            continue
        ...
        if col not in df.columns:
            # fallback: try _filtered with explicit ×100
            col_filt = f"spread_{idx}_filtered"
            if col_filt in df.columns:
                print(f"[fix_1] WARNING: using {col_filt} (×100) for {idx} — non-filtered not found")
                y = pd.to_numeric(df[col_filt], errors="coerce") * 100.0
            else:
                print(f"[fix_1] ERROR: no spread column for {idx}, skipping")
                continue
        else:
            y = pd.to_numeric(df[col], errors="coerce")

        med = float(y.abs().median())
        print(f"[fix_1] Equity {idx} ({col}): median_abs = {med:.2f} bps (target: 20-60)")
...
def stack_all_outcomes(
    ...
    print(f"\n[fix_1] Total stacked panel: {len(out):,} rows, "
    ...
    for idx in ["SPX", "NDX", "INDU"]:
        ...
        if col_filt in df.columns:
            ...
            rows.append({
                ...
                "buggy_median_abs": buggy.abs().median(),
                "fixed_median_abs": buggy.abs().median(),  # _filtered ×100 is actually correct
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("FIX 1: Unit diagnostic (before vs after)")
    print("=" * 70)
