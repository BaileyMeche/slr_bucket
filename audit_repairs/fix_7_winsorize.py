...
Clipping counts are logged to outputs/fix7_winsorize_log.csv.
"""
...
def winsorize_panel(
    ...
    if not clipped_any.empty:
        print(f"\n[fix_7_winsorize] Observations clipped at p{int(pct_lo)}/p{int(pct_hi)}:")
        ...
        print("[fix_7_winsorize] No observations clipped.")

    if log_path is not None:
        log_path.parent.mkdir(parents=True, exist_ok=True)
        log_df.to_csv(log_path, index=False)
        print(f"[fix_7_winsorize] Log saved: {log_path}")
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("FIX 7: Winsorize COVID-spike outliers")
    print("=" * 70)
    ...
    print("\n=== Conclusion ===")
    print("y_bps and y_abs_bps are now winsorized at p1/p99 within each series.")
    print("Controls are untouched. Clipping log saved to outputs/fix7_winsorize_log.csv.")
